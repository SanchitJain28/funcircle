    ...
    <div className="bg-[#131315] min-h-screen">
      ...
      {loading ? (
        ...
        <div className=" overflow-hidden ">
          <p className="text-3xl font-sans mx-4 my-4 font-bold text-white">Tickets</p>
          {tickets.map((ticket, index) => (
            <div key={index} className="my-4 ">
              <div className="flex flex-col bg-[#1D1D1F] rounded-xl m-4 border border-[#676769] p-4">
                ...
                <p className="text-lg font-sans text-white">Total spots : {ticket.capacity}</p>
              ...
              <div className=" justify-between m-4">
                <div className="flex my-2  my-2">
                  <p className="text-white">{ticket.bookedtickets} people joined</p>
                </div>
                <div className="bg-[#1D1D1F] rounded-xl px-2">
                  <div className="flex rounded-lg  items-center p-2">
                    ...
                    <p className=" font-sans  rounded-lg  text-white p-2">
                      From: {formatDate(ticket.startdatetime)}</p>
                  </div>
                  <div className="flex bg-[#1D1D1F] rounded-lg items-center p-2">
                    <Clock color="#9F9EA3" />
                    <p className=" font-sans rounded-lg text-white p-2"> To : {formatDate(ticket.enddatetime)}</p>
                  </div>
                </div>
              ...
              {ticket.venueid ?
                <div id="venue" className="m-4 rounded-lg px-4 py-4 my-4 bg-[#1D1D1F]">
                  <p className="text-white ">Venue details</p>
                  <div className="flex items-center">
                    <img src={ticket.venueid.images[0]} className="w-20 h-20 rounded-full" />
                    <div className="flex flex-col mx-4">
                      <p className="my-2 font-sans text-white">{ticket.venueid.venue_name}</p>
                      <p className=" font-sans text-white">{ticket.venueid.location}</p>
                      <div className="flex items-center w-24 bg-white px-2 py-1 rounded-lg my-1">
                        <MapPin size={20} />
                        <a className="text-black" href={ticket.venueid.maps_link}>Maps</a>
                      </div>
                    </div>
                  </div>
                </div>
              ...
              <div className="m-4">
                ...
                <p className="text-zinc-300 text-sm my-2">The event organizer has the right to change the venue, time, or date. In such cases, participants will be notified in advance.</p>
                <p className="text-zinc-300 text-sm my-2">Non-Transferable Tickets:</p>
                <p className="text-zinc-300 text-sm my-2">Tickets are non-transferable and can only be used by the person who booked them. You may be required to show ID at the venue.</p>
                <p className="text-zinc-300 text-sm my-2">Arrival & Entry:</p>
                <p className="text-zinc-300 text-sm my-2">Please arrive at the event venue on time. Late arrivals may not be allowed entry, and refunds will not be provided in such cases.</p>
                <p className="text-zinc-300 text-sm my-2">Venue Rules & Conduct:</p>
                <p className="text-zinc-300 text-sm my-2">Participants must follow all rules and guidelines set by the venue.</p>
                <p className="text-zinc-300 text-sm my-2">Any disruptive behavior may result in removal from the event without a refund.</p>
                <p className="text-zinc-300 text-sm my-2">Liability Disclaimer:</p>
                <p className="text-zinc-300 text-sm my-2">Fun Circle is a platform that facilitates ticket bookings and is not responsible for event execution, injuries, lost belongings, or disputes at the venue.</p>
                <p className="text-zinc-300 text-sm my-2">Photography & Media:</p>
                <p className="text-zinc-300 text-sm my-2">Photos and videos may be taken during the event for promotional purposes. If you prefer not to be photographed, inform the event organizer in advance.</p>
                <p className="text-zinc-300 text-sm my-2">Payment Security:</p>
                <p className="text-zinc-300 text-sm my-2">All payments are securely processed. Fun Circle does not store your payment details.</p>
                <p className="text-zinc-300 text-sm my-8">By booking a ticket, you agree to these terms. Enjoy the event! 🎉
