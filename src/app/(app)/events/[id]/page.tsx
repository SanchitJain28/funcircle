...
import EventTicketClient from "./EventsClient";
...
export default async function EventTicketPage({
  ...
  const groupIdNumber = parseInt(id, 10);
  ...
  try {
    ...
    if (!data || data.length === 0) {
      return (
        <>
          <CustomHeader />;
          <EventTicketClient group_id={id} data={[]} />
        </>
      );
    }
    ...
    const structuredData = {
      ...
      offers: validTickets.map((ticket) => ({
        ...
        url: `/events/${id}/tickets/${ticket.id}`,
      })),
      organizer: {
        "@type": "Organization",
    ...
    return (
      <>
        ...
        <div className="min-h-screen bg-[#0f0f11]">
          <CustomHeader />
          <EventTicketClient group_id={id} data={validTickets} />
        </div>
      </>
    );
  } catch (error) {
